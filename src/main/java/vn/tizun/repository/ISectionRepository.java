...
public interface ISectionRepository extends JpaRepository<SectionEntity, Long> {
    ...
    @Query("select s from SectionEntity s where s.status = :status and s.course.id = :courseId order by s.position")
    List<SectionEntity> findAllByStatusAndCourseId(@Param("courseId") Long courseId,
